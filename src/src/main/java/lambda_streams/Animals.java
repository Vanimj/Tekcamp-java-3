...
class Animals {
	...
	static List<String> capsFirst(List<String> animalList, boolean mutate) {
		if (mutate) {
			animals = animals.stream().map(x -> x.toLowerCase())
					.map(x -> x.substring(0, 1).toUpperCase() + x.substring(1)).collect(Collectors.toList());
			return animals;
		} else {
			List<String> animalListCopy = animalList.stream().map(x -> x.toLowerCase())
					.map(x -> x.substring(0, 1).toUpperCase() + x.substring(1)).collect(Collectors.toList());
	...
	static String addAnimal(String animal) {
		...
		animals = newAnimals.stream().map(x -> x.toLowerCase())
				.map(x -> x.substring(0, 1).toUpperCase() + x.substring(1)).collect(Collectors.toList());
		return animal;
	};
	...
	static List<String> lowerFirst(List<String> animalList, boolean mutate) {
		if (mutate) {
			animals = animals.stream().map(x -> x.toUpperCase())
					.map(x -> x.substring(0, 1).toLowerCase() + x.substring(1)).collect(Collectors.toList());
			return animals;
		} else {
			List<String> animalListCopy = animalList.stream().map(x -> x.toUpperCase())
					.map(x -> x.substring(0, 1).toLowerCase() + x.substring(1)).collect(Collectors.toList());
			return animalListCopy;
		}
	}
	...
	static List<String> sortAnimals(boolean mutate) {
		if (mutate) {
			animals = animals.stream().sorted((o1, o2) -> o1.compareTo(o2)).collect(Collectors.toList());
			return animals;
		} else {
			List<String> sortedanimalList = animals.stream().sorted((o1, o2) -> o1.compareTo(o2))
